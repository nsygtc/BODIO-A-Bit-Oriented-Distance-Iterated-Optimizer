...
void update_omega_second(node w[], int pn, Parameters_second para_2, int evolution_max, int iter_num, int trace_num, PlainData Data[], int HW_key[], double trace[][maxn], Paras_duplication para_dup) {
...
void update_omega_third(node w[], int pn, int evolution_max, int iter_num, int trace_num, PlainData Data[], int HW_key[], double trace[][maxn], Paras_duplication para_dup) {
	bitset<128> alpha, beta, delta, omega, omega_tmp, leader;
	...
	for (int i = 4; i <= pn; i++) {
		...
		for (int j = 0; j < 128; j++) {
			//leader
			double r_leader = (double)rand() / (double)RAND_MAX;
			if (r_leader < 0.5) {
				leader[j] = alpha[j];
			}
			else if(r_leader >= 0.5 && r_leader < 0.8){
				leader[j] = beta[j];
			}
			else
			{
			...
			//omega_tmp
			double a = 0.4 * (1 - ((double)iter_num / (double)evolution_max));
			if ((double)rand() / (double)RAND_MAX < a) {
				omega_tmp[j] = leader[j];
			}
			else {
				omega_tmp[j] = omega[j];
			}
...
void work_bGWO(int trace_num, node w[], PlainData Data[], int HW_key[], double trace[][maxn], int pn, int evolution_max, int iter_num) {
	...
	//update_omega_second(w, pn, para_2, evolution_max, iter_num, trace_num, Data, HW_key, trace, para_dup);
